...
const Page = () => {
  ...
  const initialProfiles = [
    { id: 123, name: "ANNA PHLY", image: "/profileOne.png" },
    { id: 124, name: "ANNAh PHLY", image: "/profileTwo.png" },
    { id: 125, name: "ANNA PHLY", image: "/profileThree.png" },
    { id: 126, name: "ANNA PHLY", image: "/profileFour.png" },
    { id: 127, name: "ANNA PHLY", image: "/profileFive.png" },
    { id: 128, name: "ANNA PHLY", image: "/profileSix.png" },
    { id: 129, name: "ANNA PHLY", image: "/profileSeven.png" },
    { id: 122, name: "ANNA PHLY", image: "/profileEight.png" },
  ];
  
  const moreProfiles = [
    { id: 121, name: "NEW PHLY", image: "/profileThree.png" },
    { id: 120, name: "NEW PHLY", image: "/profileOne.png" }
  ];
  ...
  return (
    <div className='text-white px-4'>
      ...
      <div className="text-center py-10">
        <h2 className="text-2xl font-bold">ENTRY <span className="text-purple-400">REQUIREMENT</span></h2>
        <div className="flex flex-col md:flex-row items-start gap-10 px-10 mt-6 text-sm">
          {categories.map((category, index) => (
            <div key={index} className={`md:w-1/2 text-left ${index === 1 ? 'lg:border-l border-purple-400 lg:pl-10' : ''}`}>
              <h3 className="text-purple-400 font-bold text-center mb-3">{category.title}</h3>
              {category.details.map((detail, idx) => (
                detail === detail.toUpperCase() ? (
                  <h4 key={idx} className="mt-4 font-bold text-white">{detail}</h4>
                ) : (
                  <p key={idx} className="mt-2">{detail}</p>
                )
              ))}
            </div>
          ))}
